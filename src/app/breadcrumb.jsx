                        ...
                        <div className="relative hero-content flex flex-col justify-center items-center">
                            ...
                            <nav aria-label="Breadcrumb" className="mb-6">
                                <ol className="flex items-center space-x-2 text-mild">
                                    <li>
                                        <Link href="/" className="hover:text-rosesecondary text-lg">Home</Link>
                                    </li>
                                    {breadcrumbs.map((crumb, index) => (
                                        <li key={crumb.href}>
                                            <span className="mx-2 text-mild">/</span>
                                            {index === breadcrumbs.length - 1 ? (
                                                <span aria-current="page">{crumb.name}</span>
                                            ) : (
                                                <Link href={crumb.href} className="hover:text-rosesecondary text-lg">
                                                    {crumb.name}
                                                </Link>
                                            )}
                                        </li>
